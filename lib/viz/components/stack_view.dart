...
class StackView extends StatelessWidget {
  ...
  /// In **bar mode**, draws a light polyline + dots through the tops of the
  /// bars. This "graph on top of the bar chart" makes the *monotonic* shape
  /// explicit (e.g. a strictly-decreasing next-greater-element stack looks like
  /// a downhill line). No effect in cell mode.
  final bool connectTops;
  ...
  const StackView({
    ...
    this.barMax,
    this.compact = false,
    this.connectTops = false,
  });

  static const double _cellWBase = 58;
  static const double _gapBase = 12;
  static const double _compactCellW = 40;
  static const double _compactGap = 6;

  /// Element width / gap, tightened in compact bar mode.
  double get _cellW =>
      compact && _barMode ? _compactCellW : _cellWBase;
  double get _gap => compact && _barMode ? _compactGap : _gapBase;
                      ...
                      points: [
                        for (var i = 0; i < n; i++)
                          Offset(
                            i * _stride + _cellW / 2,
                            baseline -
                                _barHeight((values[i] as num).toDouble(), ref),
                          ),
                      ],
