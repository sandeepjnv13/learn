...
class _VpViewState extends State<ValidParenthesesView> {
  ...
  static const List<(VizPreset, String)> _presets = [
    (
      VizPreset('Nested', detail: 'openers stack up, then unwind in reverse'),
      '([{}])',
    ),
    (
      VizPreset('Sequential', detail: 'each pair opens and closes immediately'),
      '()[]{}',
    ),
    (
      VizPreset('Wrong order',
          detail: 'counts match but nesting is interleaved → invalid',
          edgeCase: true),
      '([)]',
    ),
    (
      VizPreset('Unclosed opener',
          detail: 'stack is non-empty at the end → invalid', edgeCase: true),
      '(((',
    ),
    (
      VizPreset('Closer first',
          detail: 'a closer with an empty stack → invalid', edgeCase: true),
      '){}',
    ),
  ];

  void _loadPreset(int i) {
    _inputCtrl.text = _presets[i].$2;
    _rebuild();
  }
